    ...
    >
      ...
      {isInactive && (
        <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] flex items-center justify-center z-10 rounded-2xl">
          <div className="text-center px-4 py-2 bg-black/60 rounded-full border border-white/10">
      ...
      <div className="flex justify-between items-start relative z-10">
        ...
        <span className={`text-xs font-semibold uppercase tracking-widest px-2 py-1 rounded-full ${
          card.type === 'virtual' ? 'bg-white/10 text-white border border-white/20' : 'bg-white/20 text-white shadow-sm'
        }`}>
          {card.type}
        </span>
      </div>
      ...
      <div className="relative z-10">
        <p className="text-white/95 font-mono text-[1.15rem] tracking-[0.2em] mb-4">{card.card_number_masked}</p>
        
        <div className="flex justify-between items-end">
          <div>
            <p className="text-white/60 text-[10px] uppercase tracking-wider mb-0.5">Cardholder</p>
            <p className="text-white font-semibold text-sm tracking-wide truncate max-w-[120px]">{card.cardholder_name}</p>
          </div>
          <div className="text-right">
            <p className="text-white/60 text-[10px] uppercase tracking-wider mb-0.5">Expires</p>
            <p className="text-white font-semibold text-sm tracking-widest">{card.expiry_date}</p>
          </div>
          <div className="text-right">
            <p className="text-white/60 text-[10px] uppercase tracking-wider mb-0.5">CVV</p>
            {isInactive ? (
              <p className="text-white/40 font-mono text-sm tracking-widest">***</p>
            ) : cvv ? (
              <p className="text-white font-mono font-bold text-sm tracking-widest">{cvv}</p>
            ) : (
              <button
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <motion.div
        ...
        className="bg-[#152336] rounded-2xl p-6 w-full max-w-sm border border-white/10 shadow-2xl"
      >
        <h3 className="text-white font-bold text-lg mb-4">Set Spending Limit</h3>
        <p className="text-gray-400 text-sm mb-4">Set a daily spending limit for this card. Enter 0 for no limit.</p>
        <div className="mb-4">
          <label className="block text-gray-300 text-sm mb-1.5">Limit Amount (USD)</label>
          <input
            type="number"
            min="0"
            value={limit}
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 transition-colors">
            Cancel
          </button>
          <button
            onClick={handleSave}
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <motion.div
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        className="bg-[#152336] rounded-2xl p-6 w-full max-w-md border border-white/10 shadow-2xl"
      >
        <h3 className="text-white font-bold text-lg mb-2">Request Physical Card</h3>
        <p className="text-gray-400 text-sm mb-6">Where should we ship your new card?</p>
        
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-gray-300 text-sm mb-1.5">Full Name</label>
            <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full bg-[#0B1221] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#D4AF37]" />
          </div>
          <div>
            <label className="block text-gray-300 text-sm mb-1.5">Delivery Address</label>
            <input required type="text" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} className="w-full bg-[#0B1221] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#D4AF37]" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-gray-300 text-sm mb-1.5">City</label>
              <input required type="text" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} className="w-full bg-[#0B1221] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#D4AF37]" />
            </div>
            <div>
              <label className="block text-gray-300 text-sm mb-1.5">Postal Code</label>
              <input required type="text" value={formData.postal} onChange={e => setFormData({...formData, postal: e.target.value})} className="w-full bg-[#0B1221] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#D4AF37]" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-gray-300 text-sm mb-1.5">Country</label>
              <input required type="text" value={formData.country} onChange={e => setFormData({...formData, country: e.target.value})} className="w-full bg-[#0B1221] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#D4AF37]" />
            </div>
            <div>
              <label className="block text-gray-300 text-sm mb-1.5">Phone Number</label>
              <input required type="tel" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-[#0B1221] border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#D4AF37]" />
            </div>
          </div>

          <div className="flex gap-3 mt-6 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 transition-colors">
              Cancel
            </button>
    ...
    <div className="min-h-screen bg-[#0B1221]">
      <div className="max-w-5xl mx-auto px-4 pt-24 pb-16">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-serif text-white">My Cards</h1>
            <p className="text-gray-400 mt-1">Manage your virtual and physical cards</p>
          </div>
          <div className="flex gap-3">
            <motion.button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 ${
                hasVirtual 
                  ? 'bg-[#152336] text-[#D4AF37] border border-[#D4AF37]/30 shadow-lg cursor-not-allowed' 
        ...
        {loading ? (
          ...
          <motion.div
            ...
            className="text-center py-24 bg-[#152336] rounded-2xl border border-white/5"
          >
            <CreditCard className="w-16 h-16 text-[#D4AF37]/50 mx-auto mb-4" />
            <h3 className="text-white text-xl font-medium mb-2">No cards yet</h3>
            <p className="text-gray-400 mb-6">Request your first virtual card instantly</p>
            <motion.button
            ...
            {cards.map((card, i) => {
              ...
              return (
                <div key={card.id} className="space-y-4 max-w-sm mx-auto w-full">
                  ...
                  <div className="bg-[#152336] rounded-2xl border border-white/5 p-2 flex flex-col gap-1 shadow-lg w-full">
                    ...
                    {card.spending_limit > 0 && (
                      <div className="px-4 py-3 bg-white/5 rounded-xl mb-1">
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-gray-400 font-medium">Spending Limit</span>
                          <span className="text-white font-medium">${Number(card.spending_limit).toLocaleString()}</span>
                        </div>
                      </div>
                    )}
                    ...
                    >
                      ...
                      {actionLoading === card.id ? <Loader2 size={16} className="animate-spin text-gray-500" /> : <ChevronRight size={16} className="text-gray-500 group-hover:text-white transition-colors" />}
                    </button>
