    ...
    <div className="min-h-screen bg-[#0B1221] text-white font-sans">
      <div className="max-w-6xl mx-auto space-y-8 p-6 md:p-10">
        ...
        <header className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
          <div>
            ...
            <p className="text-gray-400">Manage your Krevon international accounts</p>
          </div>
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            <Link to="/fund-account" className="flex-1 sm:flex-none justify-center flex items-center gap-2 bg-[#152336] hover:bg-[#1e3048] border border-[#1e3048] text-white px-4 sm:px-5 py-2.5 rounded-lg font-medium transition-colors whitespace-nowrap">
              <Plus size={18} />
              Add Funds
            </Link>
            <Link to="/cards" className="w-full sm:w-auto justify-center flex items-center gap-2 bg-[#152336] hover:bg-[#1e3048] border border-[#1e3048] text-white px-4 sm:px-5 py-2.5 rounded-lg font-medium transition-colors whitespace-nowrap">
              <CreditCard size={18} />
              My Cards
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#152336] rounded-2xl p-6 border border-gray-800 md:col-span-2 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-8 relative z-10">
              <div>
                <p className="text-gray-400 text-sm font-medium mb-1">Total Balance (USD Eqv)</p>
              ...
              <div className="p-3 bg-[#0B1221] rounded-xl text-[#D4AF37]">
                <Activity size={24} />
              </div>
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-400 relative z-10">
              <span className="inline-block w-2 h-2 rounded-full bg-green-500"></span>
              Account Number: <span className="font-mono text-gray-300">{account?.account_number || 'â€¢â€¢â€¢â€¢ â€¢â€¢â€¢â€¢ â€¢â€¢â€¢â€¢'}</span>
            </div>
          </div>

          <div className="bg-[#152336] rounded-2xl p-6 border border-gray-800 flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-semibold text-white">Currency Wallets</h3>
              <button
            ...
            <div className="flex-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
              {account?.balances && account.balances.length > 0 ? (
                account.balances.map((wallet) => (
                  <div key={wallet.currency_code} className="flex justify-between items-center p-3 bg-[#0B1221] rounded-xl">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#152336] flex items-center justify-center text-xs font-bold text-[#D4AF37]">
        ...
        <div className="bg-[#152336] rounded-2xl p-6 border border-gray-800">
          <div className="flex justify-between items-center mb-6">
          ...
          <div className="space-y-3">
            {recentTransactions.length > 0 ? (
              recentTransactions.map((tx) => (
                <div key={tx.id || tx._id} className="flex items-center justify-between p-4 bg-[#0B1221] rounded-xl hover:bg-[#0f1929] transition-colors border border-transparent hover:border-gray-800">
                  ...
                  <div className={`font-mono font-medium ${tx.type === 'deposit' || tx.amount > 0 || tx.type === 'bank_funding' ? 'text-green-500' : 'text-white'}`}>
              ...
              <div className="text-center py-8 text-gray-500 bg-[#0B1221] rounded-xl">
      ...
      {showWalletModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowWalletModal(false)}>
          <div className="bg-[#152336] rounded-2xl border border-gray-800 w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center p-6 border-b border-gray-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">Add Currency Wallet</h3>
                  <p className="text-xs text-gray-400">Select a currency to create a new wallet</p>
                </div>
              </div>
              <button onClick={() => setShowWalletModal(false)} className="p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                <X size={20} />
              </button>
            </div>
              ...
              {AVAILABLE_CURRENCIES
                ...
                .map(c => (
                  <button
                    ...
                    className="p-4 bg-[#0B1221] hover:bg-[#0f1929] border border-gray-800 hover:border-[#D4AF37]/40 rounded-xl text-left transition-all disabled:opacity-50 group"
                  >
                    <div className="text-[#D4AF37] font-bold text-lg mb-1 group-hover:text-[#F3D566] transition-colors">{c.symbol}</div>
                    <div className="text-sm font-medium text-white">{c.code}</div>
